    ...
    def insert_feeding_record(
        ...
        try:
            with self._get_connection() as conn:
                with conn.cursor() as cursor:
                    cols = (
                        f"{Constants.COLUMN_FOOD_WEIGHT}, amount, "
                        f"{Constants.COLUMN_STATUS_MOUNT}, motor, "
                        f"{Constants.COLUMN_TIMESTAMP}"
                    )
                    query = f"""
                    ...
                    self.logger.debug(
                        f"Inserted feeding record: weight={food_weight}g, amount={amount}g"
                    )
                    return True
